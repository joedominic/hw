...
def purge_generated_user_resumes(retention_days: int) -> int:
    """
    ...
    ).values_list("original_resume_id", flat=True)

    qs = (
        UserResume.objects.filter(is_library=False, uploaded_at__lt=cutoff)
        .exclude(id__in=active_resume_ids)
